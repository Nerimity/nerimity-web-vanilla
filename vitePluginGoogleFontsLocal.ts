...
interface GoogleFontsLocalOptions {
  icons: string[];
}

export function googleFontsLocal(options: GoogleFontsLocalOptions): Plugin {
  let inlinedCss = "";

  function buildGoogleFontsUrl(): string {
    const sorted = [...options.icons].sort();
    const iconNames = sorted.join(",");
    return `https://fonts.googleapis.com/css2?family=Material+Symbols+Rounded:FILL@0..1&icon_names=${iconNames}&display=swap`;
  }
